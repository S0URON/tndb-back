...
export async function addMovieToFavourites(req, res) {
    try {
        const body = {
        ...
        user.watchList.forEach(watchedMovie => {
            if(watchedMovie._id.toString() === movie._id.toString())
                watchedMovie.isFavourite = !watchedMovie.isFavourite;
        });
        
        await user.save();

        return res.status(200).json({ message: "movie added to favourites" });
    } catch (error) {
